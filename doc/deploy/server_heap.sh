...
function init_apm() {
  if [[ ${ENABLE_APM} = "on" ]]; then
      APM_OPTS="-javaagent:/opt/pinpoint/pinpoint-agent/pinpoint-bootstrap-2.3.3.jar
            -Dpinpoint.agentId=${APP_NAME}_${ENV}
            -Dpinpoint.applicationName=${APP_NAME}_${ENV}"
...
# 设置环境, 与应用配置 bootstrap.yml 的 ${FKH_NAME_SPACE} 对应, 此处设置的变量会被 spring 在启动时替换
# 使用 -DIDENTIFY 来区分应用 (由于服务器资源有限, 一台服务器可能会部署同一个应用, 只是环境不同而已)
# 重写 fkh.logging.file.path 配置
function running() {
  ...
  echo_green "启动命令:
    nohup ${JAVA_EXE} -jar
      ...
      -DFKH_NAME_SPACE=${NAME_SPACE_PREFIX}-${ENV}
      -Ddeploy.path=${DEPLOY_DIR}
      -Dstart.type=shell
      -Dconfig.path=${DEPLOY_DIR}/config/
      -Dfkh.logging.file.path=${LOG_PATH}/${ENV}
      -Dfkh.logging.file.name=${LOG_NAME}
      -Djar.file=${JAR_FILE}
      ${JMX_OPTIONS}
      ${DEBUG_OPTS}
      ${APM_OPTS}
...
function show_info() {
  ...
  echo -e "\033[33m FKH_NAME_SPACE: fkh-${ENV} \033[0m"
  echo -e "\n"
}
...
echo_green "          \/            \/       \/                      \/             \/      \/      \/      \/           "
echo_green "                                                                                                             "
echo_green "                                         :: FKH Boot Startup Script ::                                       "
echo
...
FINAL_LOG_PATH=""
# 自定义 JVM 参数
JVM_OPTIONS="-Xms1024M -Xmx1024M -XX:MetaspaceSize=512M -XX:MaxMetaspaceSize=1024M "
# 自定义参数, 先设置变量
...
-n:重写 FKH_NAME_SAPCE 参数\texp: bin/server.sh -s dev -n 'fkh'\t\t(重写的前缀, 最终为 {prefix}-{ENV}, 请不要添加 "-")
\033[0m"
    exit 1
    ;;
  \?)
